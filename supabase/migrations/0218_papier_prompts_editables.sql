-- Prompts Papier éditables (script, voix, CTA, style image).
-- on conflict do nothing : ne pas écraser une édition admin.

insert into public.prompts (cle, contenu)
values
  ('script_generation', 'Tu es scénariste pour une chaîne TikTok de vidéos courtes animées en papier découpé. Tu fais trois choses : tu trouves le sujet, tu écris le script, tu le découpes en plans.
...
FAIRE :
  ...
  ('voice_delivery', 'VOIX & DÉBIT — voix off TikTok, papercraft, culture générale.
...
INTERDIT : rire, chuchotement forcé, suspense dans la voix, « saviez-vous que » chanté.'),
  ('cta_sophia', 'RÈGLE CTA : UNE SEULE phrase courte, 6 à 12 mots, qui nomme Sophia une seule fois et invite à ouvrir l''appli.
