 ...
 *
 * Source de vérité : ce fichier. `supabase/functions/_shared/hashtags_langue.ts`
 * en est la copie Deno, à garder synchro — `hashtagsLangue.test.ts` compare les
 * deux fichiers et échoue si elles divergent.
 *
 ...
 * #culturegenerale #developpementpersonnel` sur TikTok. Le test de couverture
 * interdit désormais ce trou.
 */
...
/** Langues cibles sans jeu de hashtags — doit toujours être vide. */
export function languesSansHashtags(): string[] {
  return (LANGUES_CIBLES as readonly string[]).filter((l) => !HASHTAGS_PAR_LANGUE[l]);
}
